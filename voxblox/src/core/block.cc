...
// (Todo Jianhao) Note that we only serialize/deserialize the large voxel, to
// get full voxel information, we currently can only use the fucntion
// "outputVoxelsToTxt"
template <>
void Block<TsdfVoxel>::deserializeFromIntegers(
    const std::vector<uint32_t>& data) {
  // constexpr size_t kNumDataPacketsPerVoxel = 3u;
  size_t kNumDataPacketsPerVoxel = 5u;  // plus 2 for best semantic
  const size_t num_data_packets = data.size();
  bool semantic_loaded = true;

  if (num_voxels_ * 3u != num_data_packets) {
    CHECK_EQ(num_voxels_ * kNumDataPacketsPerVoxel, num_data_packets);
  } else {
    kNumDataPacketsPerVoxel = 3u;
    CHECK_EQ(num_voxels_ * kNumDataPacketsPerVoxel, num_data_packets);
  ...
  for (size_t voxel_idx = 0u, data_idx = 0u;
    ...
    if (semantic_loaded) {
      ...
      voxel.labels.resize(1);
      voxel.probabilities.resize(1);
      voxel.labels[0] = static_cast<uint32_t>(bytes_4);
      memcpy(&(voxel.probabilities[0]), &bytes_5, sizeof(bytes_5));
    }
  }
}
...
void Block<TsdfVoxel>::serializeToIntegers(std::vector<uint32_t>* data) const {
  CHECK_NOTNULL(data);
  // constexpr size_t kNumDataPacketsPerVoxel = 3u;
  constexpr size_t kNumDataPacketsPerVoxel = 5u;  // plus 2 for best semantic
  data->clear();
  ...
  for (size_t voxel_idx = 0u; voxel_idx < num_voxels_; ++voxel_idx) {
    ...
    if (voxel.labels.size() > 0) {
      SemanticProbabilities::const_iterator most_likely = std::max_element(
          voxel.probabilities.begin(), voxel.probabilities.end());

      data->push_back(static_cast<uint32_t>(voxel.labels[std::distance(
          voxel.probabilities.begin(), most_likely)]));
      const uint32_t* bytes_probability_ptr =
          reinterpret_cast<const uint32_t*>(&(voxel.probabilities[std::distance(
              voxel.probabilities.begin(), most_likely)]));
      data->push_back(*bytes_probability_ptr);
    } else {
      // make them all zero to represent no semantic label
      data->push_back(static_cast<uint32_t>(0));
      data->push_back(static_cast<uint32_t>(0));
    }
  }

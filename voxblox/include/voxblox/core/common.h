...
typedef std::vector<u_int32_t> Semantics;
typedef std::vector<float> SemanticProbabilities;
...
inline GlobalIndex getParentVoxelIndexFromGlobalVoxelIndex(
    const GlobalIndex& global_voxel_idx,
    FloatingPoint child_voxels_per_side_inv) {
  return GlobalIndex(
      std::floor(static_cast<FloatingPoint>(global_voxel_idx.x()) *
                 child_voxels_per_side_inv),
      std::floor(static_cast<FloatingPoint>(global_voxel_idx.y()) *
                 child_voxels_per_side_inv),
      std::floor(static_cast<FloatingPoint>(global_voxel_idx.z()) *
                 child_voxels_per_side_inv));
...
inline int getLocalChildVoxelIndexFromGlobalVoxelIndex(
    const GlobalIndex& global_voxel_idx, const int voxel_size_ratio) {
  // add a big number to the index to make it positive
  constexpr int offset = 1 << (8 * sizeof(IndexElement) - 1);

  CHECK(isPowerOfTwo(voxel_size_ratio));

  return ((global_voxel_idx.x() + offset) & (voxel_size_ratio - 1)) +
         ((global_voxel_idx.y() + offset) & (voxel_size_ratio - 1)) *
             voxel_size_ratio +
         ((global_voxel_idx.z() + offset) & (voxel_size_ratio - 1)) *
